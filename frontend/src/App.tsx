...
export default function App() {
  ...
  return <><header className="topbar"><button className="brand" onClick={() => void resetFlow()}><span className="brand-mark">R²</span><span>Rubik's 2×2<br /><small>Camera Solver</small></span></button><span className="local-badge">Local only</span></header>
    <main>
      ...
      {state.screen === "welcome" && <section className="hero"><div className="hero-copy"><span className="eyebrow">Shortest solution · HTM</span><h1>Scan your 2×2.<br />Turn it solved.</h1><p>Hold each face inside the guide. Usable faces capture quickly, show their recognized colors immediately, and stay local.</p><div className="hero-actions"><button className="primary large" disabled={state.busy} onClick={() => void begin("permission")}>Start scanning</button><button disabled={state.busy} onClick={() => void begin("verify", DEMO_FACELETS, true)}>Try demo without camera</button><button disabled={state.busy} onClick={() => void begin("verify", SOLVED_FACELETS, true)}>Enter manually</button></div><ul className="feature-list"><li>Automatic guided capture</li><li>Physically valid cube checks</li><li>Optimal 2×2 solution</li></ul></div><div className="hero-cube" aria-hidden="true"><div className="cube-art"><i /><i /><i /></div><p>6 faces · 24 facelets · one exact state</p></div></section>}
      {state.screen === "permission" && <section className="center-card panel"><span className="eyebrow">Camera setup</span><h1>{state.retakeFace ? `Retake ${state.retakeFace}` : "Allow camera access"}</h1><p>Tap below to start Safari's rear camera. Only the square face snapshot is sent to the local backend, then discarded.</p>{camera.problem && <div className="alert error" role="alert">{camera.problem.message}</div>}<button className="primary large" disabled={state.busy || camera.starting} onClick={() => void requestCamera()}>{state.busy || camera.starting ? "Requesting…" : state.retakeFace ? "Start camera to retake" : "Allow camera"}</button><button onClick={() => patch({ screen: "scan", cameraFree: true })}>Use image files instead</button><p className="privacy-note">Rear camera is preferred. Permission begins only from this button.</p></section>}
      {state.screen === "scan" && <section className="scan-layout"><div className="scan-copy panel"><span className="eyebrow">{state.retakeFace ? "Retaking captured face" : `Face ${capturedCount + 1} of 6`}</span><h1>{scanFace} · {faceName(scanFace)}</h1><p>{SCAN_INSTRUCTIONS[scanFace]}</p><div className="scan-progress" aria-label={`${capturedCount} faces captured`}>{SCAN_ORDER.map((face) => <span key={face} className={state.capturedFaces[face] ? "done" : face === scanFace ? "current" : ""}>{face}</span>)}</div><details><summary>Orientation details</summary><p>Rotate the whole cube only. Never turn a layer while scanning.</p></details>{state.captureFeedback && <p className="capture-success" role="status">✓ {state.captureFeedback}</p>}<CapturedFaces previews={state.capturedFaces} activeFace={state.retakeFace} busy={state.busy} onRetake={startRetake} /></div><div><CameraCapture stream={camera.stream} devices={camera.devices} problem={camera.problem} busy={state.busy} captureKey={scanFace} autoEnabled={state.autoCapture} onAutoChange={(autoCapture) => patch({ autoCapture })} onCapture={(blob, source) => void uploadCapture(blob, source)} onSwitchCamera={(id) => void requestCamera(id)} onRecover={() => void requestCamera()} onPlaybackProblem={camera.reportPlaybackProblem} />{state.notices.length > 0 && <div className="alert warning"><ul>{state.notices.map((notice) => <li key={notice.code}>{notice.message}</li>)}</ul></div>}</div></section>}
      {state.screen === "processing" && <section className="center-card panel processing"><span className="processing-spinner" aria-hidden="true" /><span className="eyebrow">Six faces captured</span><h1>Checking cube…</h1><p>Validating the physical state and finding the shortest solution.</p></section>}
