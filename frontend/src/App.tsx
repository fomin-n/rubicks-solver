...
export default function App() {
  ...
  return <><header className="topbar"><button className="brand" onClick={() => void resetFlow()}><span className="brand-mark">R²</span><span>Rubik's 2×2<br /><small>Camera Solver</small></span></button><span className="local-badge">Local only</span></header>
    <main>
      ...
      })}</div><div className={`scan-feedback-slot${state.notices.length ? " warning" : state.captureFeedback ? " success" : ""}`} role="status"><span>{state.notices[0]?.message ?? (state.captureFeedback ? `✓ ${state.captureFeedback}` : "Keep the full face inside the guide")}</span></div><details><summary>Orientation details</summary><p>Rotate the whole cube only. Never turn a layer while scanning.</p></details></div><div className="scan-camera-column"><CameraCapture stream={camera.stream} devices={camera.devices} problem={camera.problem} busy={state.busy} captureKey={scanFace} autoEnabled={state.autoCapture} onAutoChange={(autoCapture) => patch({ autoCapture })} onCapture={(blob, source) => void uploadCapture(blob, source)} onSwitchCamera={(id) => void requestCamera(id)} onRecover={() => void requestCamera()} onPlaybackProblem={camera.reportPlaybackProblem} /><div className="scan-preview-dock"><CapturedFaces compact previews={state.capturedFaces} activeFace={state.retakeFace} busy={state.busy} onRetake={startRetake} /></div></div></section>}
